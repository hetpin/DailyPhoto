...
public class GridViewHeader extends android.widget.GridView implements
	...
	public void onDetachedFromWindow() {
		if (originalAdapter != null) {
			originalAdapter
					.unregisterDataSetObserver(originalAdapterDataSetObserver);
		}
	...
	public void setAdapter(ListAdapter a) {
		...
		// Registramos un DataSetObserver para que cuando se produzcan cambios
		// en el adaptador original lo podamos reflejar
		// en el fakeAdapter
	...
	public class mListAdapter extends BaseAdapter {
		@Override
		public int getCount() {
			return originalAdapter.getCount() > 0 ? originalAdapter.getCount()
					+ GridViewHeader.this.getNumColumnsCompat() : 0;
		}

		@Override
		public Object getItem(int position) {
			return originalAdapter.getItem(position
					+ GridViewHeader.this.getNumColumnsCompat());
		}
		...
		public View getView(int position, View convert, ViewGroup parent) {
			if (position < GridViewHeader.this.getNumColumnsCompat()) {
				if (convert == null) {
					...
					ViewGroup.LayoutParams mParams = new LayoutParams(
				...
				int realPosition = position
						- GridViewHeader.this.getNumColumnsCompat();
				convert = originalAdapter
						.getView(realPosition, convert, parent);
			}
			return convert;
		}
	};
	 ...
	 *            foo
	 */
	public void addHeaderView(final View v, final Object data,
	...
	private void setupView(View v) {
		boolean isLayedOut = !((v.getRight() == 0) && (v.getLeft() == 0)
				&& (v.getTop() == 0) && (v.getBottom() == 0));
		...
		v.measure(
				MeasureSpec.makeMeasureSpec(mDisplayWidth, MeasureSpec.AT_MOST),
				MeasureSpec.makeMeasureSpec(0, MeasureSpec.UNSPECIFIED));
		v.layout(0, getTotalHeaderHeight(), v.getMeasuredWidth(),
				getTotalHeaderHeight() + v.getMeasuredHeight());
	}
	...
	private void drawHeaders() {
		if (mHeaderViewInfo != null) {
			...
			// Para evitar ciclos infinitos de onDraw / drawHeaders porque si en
			// onDraw le pongo el topMargin efectúa
			// repintado, entonces llama a drawHeaders y así....
			if (lastPos != startPos && !setFixed && bringToFront) {
				if (mScrollOfsset <= headerViewHeight) {
					RelativeLayout.LayoutParams mParams = (android.widget.RelativeLayout.LayoutParams) mHeaderViewInfo.view
							.getLayoutParams();
	...
	public void onScroll(AbsListView view, int firstVisibleItem,
		...
		if (this.getAdapter() != null) {
			...
			if (count > this.getNumColumnsCompat()) {
				...
				mScrollOfsset = ((firstVisibleItem / this.getNumColumnsCompat()) * child
						.getMeasuredHeight())
						+ totalHeaderHeight
						- child.getTop() + this.getVerticalSpacingCompat();
			}
		}
	...
	public void onItemClick(AdapterView<?> adapter, View view, int position,
		...
		if (clickListenerFromActivity != null) {
			clickListenerFromActivity.onItemClick(adapter, view, position
					- GridViewHeader.this.getNumColumnsCompat(), id);
		}
	}
}